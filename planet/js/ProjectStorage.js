...
class ProjectStorage {
    ...
    async restore() {
        ...
        try {
            ...
            console.error("[ProjectStorage] Primary data corrupted, attempting backup restore:", e);
            this.data = null;
        }

        // If primary data is missing or corrupted, attempt recovery from backup.
        if (this.data === null || this.data === undefined) {
            try {
                const backupData = await this.get(this.BackupStorageKey);
                if (backupData !== null && backupData !== undefined) {
                    this.data =
                        typeof backupData === "string" ? JSON.parse(backupData) : backupData;
                    console.warn("[ProjectStorage] Restored from backup successfully.");
                    // Re-persist recovered data as primary.
                    await this.set(this.LocalStorageKey, this.data);
                }
            } catch (backupError) {
    ...
    async initialiseStorage() {
        const hadData = this.data !== null && this.data !== undefined;
        ...
        // Only persist if we already had data or this is a genuine first run.
        // Avoid overwriting backup with empty data on corruption.
        if (hadData) {
            await this.save();
        }
    }
        ...
        this._autoSaveInterval = setInterval(async () => {
            if (this.data !== null && this.data !== undefined) {
                try {
                    ...
                    console.error("[ProjectStorage] Auto-save failed:", e);
                }
            }
        }, intervalMs);
    }
